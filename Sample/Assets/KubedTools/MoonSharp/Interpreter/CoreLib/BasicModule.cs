...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class BasicModule
	{
		...
		[MoonSharpModuleMethod]
		public static DynValue button(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			//Debug.Log("button");
			int x = 0; int y = 0;
			...
			int.TryParse(args.AsStringUsingMeta(executionContext, 0, "x"), out x);
			int.TryParse(args.AsStringUsingMeta(executionContext, 1, "y"), out y);

			executionContext.GetScript().Options.DebugPrint("x=" + x + " y=" + y);

			List<DynValue> values = new List<DynValue>();
			DynValue v_num = args.AsType(0, "button", DataType.String, false);
